        ...
        while True: # while loop
            ...
            try:
                ...
                for (datarow) in cursor:
                    ...
                    payload = payload + '"prodcode":' + ('null' if datarow[2] == None else ('"' + str(datarow[2]) +'"') ) +  ','
                    payload = payload + '"prodname":' + ('null' if datarow[3] == None else ('"' + str(datarow[3]) +'"') )  + ','
                    payload = payload + '"prodtype":' + ('null' if datarow[4] == None else ('"' + str(datarow[4]) +'"') ) + ','
                    payload = payload + '"prodspec":' + ('null' if datarow[5] == None else ('"' + str(datarow[5]) +'"') ) + ','
                    payload = payload + '"proddesp":' + ('null' if datarow[6] == None else ('"' + str(datarow[6]) +'"') ) + ','
                    payload = payload + '"quan":' + ('null' if datarow[7] == None else ('"' + str(datarow[7]) +'"') ) + ','
                    payload = payload + '"unit":' + ('null' if datarow[8] == None else ('"' + str(datarow[8]) +'"') ) + ','
                    payload = payload + '"pkg":'+ ('null' if datarow[9] == None else ('"' + str(datarow[9]) +'"') ) + ','
                    payload = payload + '"recipaddr":' + ('null' if datarow[10] == None else ('"' + str(datarow[10]) +'"') ) + ','
                    payload = payload + '"recipname":' + ('null' if datarow[11] == None else ('"' + str(datarow[11]) +'"') ) + ','
                    payload = payload + '"recipphone1":' + ('null' if datarow[12] == None else ('"' + str(datarow[12]) +'"') ) + ','
                    payload = payload + '"recipphone2":' + ('null' if datarow[13] == None else ('"' + str(datarow[13]) +'"') ) + ','
                    payload = payload + '"recipemail":' + ('null' if datarow[14] == None else ('"' + str(datarow[14]) +'"') ) + ','
                    payload = payload + '"assignto":' + ('null' if datarow[15] == None else ('"' + str(datarow[15]) +'"') ) + ','
                    payload = payload + '"delicode":' + ('null' if datarow[16] == None else ('"' + str(datarow[16]) +'"') ) + ','
                    payload = payload + '"delidesp":' + ('null' if datarow[17] == None else ('"' + str(datarow[17]) +'"') ) + ','
                    payload = payload + '"delimemo":' + ('null' if datarow[18] == None else ('"' + str(datarow[18]) +'"') ) + ','
                    payload = payload + '"srcip":' + ('null' if datarow[19] == None else ('"' + str(datarow[19]) +'"') ) + ','
                    payload = payload + '"sender":"' + macaddr + '",'
                    payload = payload + '"netid":"' + netid  + '",'
                    payload = payload + '"progid":"' + progid + '",'
                    payload = payload + '"updtime":' + ('null' if datarow[20] == None else ('"' + str(datarow[20]).replace(' ','T') + str(tzone1) +'"') ) + ','
                    payload = payload + '"rectime":' + ('null' if datarow[21] == None else ('"' + str(datarow[21]).replace(' ','T') + str(tzone1) +'"') ) + ','
                    payload = payload + '"pushflag":' + ('null' if datarow[22] == None else ('"' + str(datarow[22]) +'"') ) 
                    payload = payload + '}' 
